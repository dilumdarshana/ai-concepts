...
- Concept routes: `/messages`, `/prompt`, `/chat-prompt`, `/structured`, `/chain`, `/lc`, `/stream`, `/tools`, `/memory`, `/trim`, `/judge`
...
- `/judge` = LLM-as-judge: `judge.ts` grades the answer and writes a score to the trace via `@langfuse/client`; see `LANGFUSE.md` §LLM-as-Judge
